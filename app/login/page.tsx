...
import { login, loginMagicLink } from './actions';
...
function LoginFormContent() {
  ...
  const handleMagicSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const res = await loginMagicLink(null, formData);
      if (res) {
        if (res.success) {
    ...
    <div className="w-full max-w-md p-8 rounded-2xl border border-border-custom bg-surface/50 backdrop-blur-xl shadow-2xl shadow-accent/5 relative overflow-hidden">
      ...
      <div className="text-center mb-8 relative z-10">
        ...
        <p className="text-xs text-muted-custom mt-2">
          Enter your details below to access your Silex workspace.
        </p>
      </div>
      ...
      {/* Tabs */}
      <div className="flex p-1 rounded-xl bg-background border border-border-custom mb-6 relative z-10">
        <button
        ...
        <form onSubmit={handlePasswordSubmit} className="space-y-5 relative z-10">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-xs font-semibold text-muted-custom">Password</label>
              <Link
                href="/forgot-password"
                className="text-xs font-semibold text-accent hover:opacity-90 transition-opacity"
              >
                Forgot Password?
              </Link>
            </div>
      ...
      {/* Magic Link Form */}
      {activeTab === 'magic' && (
        <form onSubmit={handleMagicSubmit} className="space-y-5 relative z-10">
          <div>
            <label className="block text-xs font-semibold text-muted-custom mb-2">Email Address</label>
            <input
              type="email"
